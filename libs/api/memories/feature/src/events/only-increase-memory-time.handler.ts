...
import { OnlyIncreseMemoryTimeEvent } from '@mp/api/memories/util';
import { EventsHandler, IEventHandler } from '@nestjs/cqrs';
import { ReviveStatus } from '@mp/api/memories/util';

@EventsHandler(OnlyIncreseMemoryTimeEvent)
export class OnlyIncreseMemoryTimeEventHandler implements IEventHandler<OnlyIncreseMemoryTimeEvent> {
  constructor(
    private readonly memoryRepository: MemoriesRepository,
    private readonly usersRepository: UsersRepository,
  ) {}

  async handle(event: OnlyIncreseMemoryTimeEvent) {
    console.log(`${OnlyIncreseMemoryTimeEventHandler.name}`);
